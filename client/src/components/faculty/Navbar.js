...
function Navbar({facultyID, content}) {
    const navigate = useNavigate();
    const handleLogout = async (e) => {
        e.preventDefault();
        try {
          await axios.post('http://localhost:3500/faculty/logout');
    ...
    return (
        <nav className="py-2  top-0 z-10 bg-white backdrop-filter backdrop-blur-lg bg-opacity-10 border-b border-gray-700 flex items-center justify-between py-1">
            ...
            <div className='flex items-center justify-between'>
                <p className = "text-center mx-auto text-white">Role: Faculty</p>
            </div>
            <div className='flex items-center'>
                <form className='flex items-center'>
                    <button type="submit" class="text-white border border-gray-300 focus:outline-none hover:bg-gray-700 font-medium rounded-full text-sm px-5 py-2.5 me-2 mb-2  " onClick ={() => navigate(`/faculty/${facultyID}/newCourse`, { state : { id: content._id, facultyName: content.name}})}>Create Course</button>
                </form>
